...
const reducer: Reducer<OrdersState, Actions.ACTIONS> = (state = initialState, action) => {
  switch (action.type) {
    ...
    case Actions.PRODUCT_EDIT_CLICKED: return { ...state, productInEdit: { ...action.product } };
    case Actions.PRODUCT_EDIT_CANCELED: case Actions.PRODUCT_SAVED: return { ...state, productInEdit: undefined, originalProduct: undefined };
